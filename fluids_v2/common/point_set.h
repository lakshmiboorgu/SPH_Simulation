...
using namespace glm;
	...
	public:
		...
		double Test_RayPolyIntersect(glm::vec3 const& P0, glm::vec3 const& V0, glm::vec3 const& p11,   glm::vec3 const& p21,glm::vec3 const& p31);
		// Point Sets
	...
	//	virtual float GetValue ( float x, float y, float z, Vector3DF& dir );
		...
		// Misc
		virtual int AddVolume ( Vector3DF min, Vector3DF max, float spacing );
		virtual void AddControlVolume (std::vector<glm::vec3> *myvertices );
	...
	protected:
		...
		std::vector< int >			m_GridControlCnt;
		...
		// Neighbor Table
		unsigned short				m_NC[65536];			// neighbor table (600k) //stores no of neigbours
		unsigned short				m_Neighbor[65536][MAX_NEIGHBOR];	
		float						m_NDist[65536][MAX_NEIGHBOR];
	
		// Control Neighbor Table
		unsigned short				m_CNC[65536];			// neighbor table (600k) //stores no of neigbours
		unsigned short				m_CNeighbor[65536][MAX_NEIGHBOR];	
		float						m_CNDist[65536][MAX_NEIGHBOR];
