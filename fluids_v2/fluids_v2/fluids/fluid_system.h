...
#pragma once
	...
	public:
		FluidSystem ();
		std::vector<std::vector<glm::vec3>> *myvertices; 
		std::vector<std::vector<float>> allw;
		...
		void SPH_DrawDomain ();
		void SPH_DrawInitBoundary();
		void SPH_ComputeKernels ();
		void CreateMeanValueCoordinates();

		void SPH_ComputePressureSlow ();			// O(n^2)
		void SPH_ComputePressureGrid ();			// O(kn) - spatial grid
		
		Vector3DF SPH_ComputeTControlForces(Vector3DF pos,Vector3DF vlc,int index,std::vector<Vector3DF> allvi);           //calculate the control forces
